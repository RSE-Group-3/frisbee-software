...
tmux send-keys -t $SESSION:0.0 "ros2 launch fb_gazebo sim.launch.py" C-m # launch gazebo

tmux send-keys -t $SESSION:0.1 "ros2 run teleop_twist_keyboard teleop_twist_keyboard" C-m

tmux send-keys -t $SESSION:0.2 "ros2 node info /diff_drive_controller" C-m
